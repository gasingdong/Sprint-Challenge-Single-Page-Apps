...
> Answer: The benefit of using `client-side routing` is that we do not have to make requests to the server and wait for responses in order to render different content based on URLs.
- [ ] What does AJAX stand for?
> Answer: Asynchronous Javascript and XML
- [ ] What are `controlled components` in React?
> Answer: `controlled components` are components that are controlled by state in React, so that we are in control of the data through state rather than DOM elements like form input.
